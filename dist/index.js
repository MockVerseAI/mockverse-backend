...
const limiter = rateLimit({
    ...
    handler: (_, __, ___, options) => {
        throw new ApiError(options.statusCode || 500, `There are too many requests. You are only allowed ${options.limit} requests per ${options.windowMs / 60000} minutes`);
    },
});
app.use(limiter);
...
app.use(session({
    secret: process.env.EXPRESS_SESSION_SECRET || "session-secret",
    resave: true,
